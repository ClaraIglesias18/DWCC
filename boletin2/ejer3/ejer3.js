...
if (Number(nombre)) {
    ...
    let i = 0;
    while (nombres[i] != nombre && i < nombres.length - 1) {
        i++;
    }
    ...
    if (nombres[i] == nombre) {
        ...
        for (let k = i + 1; k <= nombres.length - 1; k++) {
            if (nombres[k] == nombre) {
                countMod++;
                modulosImparte += modulos[k] + " ";
                sumaAlumnos += alumnos[k];
            }
        }
...
salida +=
    "\nEl profesor " +
    nombres[posMayorNumAlum] +
    " es el que tiene mayor numero de alumnos matriculados en el modulo " +
    modulos[posMayorNumAlum] +
    " con un numero de " +
    alumnos[posMayorNumAlum] +
    " alumnos.";
